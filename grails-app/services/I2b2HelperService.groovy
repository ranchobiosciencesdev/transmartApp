...
//		            """;
    ...
    def List<AcrossTrialsOntologyTerm> getAllXTrialsLeafNodes(AcrossTrialsOntologyTerm top){
        ...
        top.children.each { child ->
            nodes.addAll(getAllXTrialsLeafNodes(child))
        }
...
//		ON a.cat=b.cat ORDER BY a.cat""";
    ...
    def getSampleHeatMapData(String pathwayName,
        ...
        try {
            ...
            sql.eachRow(sqlStr) { row ->
                ...
                if (curGeneSymbol == null || curGeneSymbol.equals(geneSymbol) == false) {
                    // reached next gene
                    if (assayIdValueMap != null) {
                        // Output data from previous gene
    ...
    def getSurvivalAnalysisData(List<String> concepts1, List<String> concepts2, List<String> subjects1, List<String> subjects2,
        ...
        if (concepts1 == null || concepts1.size() == 0) {
            throw new Exception("The subset 1 has empty concepts")
        };
        if (concepts2 == null || concepts2.size() == 0) {
            throw new Exception("The subset 2 has empty concepts")
        };

        if (subjects1 == null || subjects1.size() == 0) {
            throw new Exception("The subset 1 has no subjects")
        };
        if (subjects2 == null || subjects2.size() == 0) {
            throw new Exception("The subset 2 has no subjects")
        };

        if (saFiles == null) {
            throw new Exception("The object saFiles cannot be null")
        };
    ...
    void getSNPViewerDataByProbe(String subjectIds1, String subjectIds2, List<Long> geneSearchIdList, List<String> geneNameList,
        ...
        if (geneSnpPageBuf == null) {
            throw new Exception("The geneSnpPageBuf object is not instantiated")
        };
    ...
    void getSNPDataByProbeByChrom(List<SnpDataset> datasetList, Map<String, List<SnpDataByProbe>> snpDataByChromMap,
        ...
        if (snpDataByChromMap == null) {
            throw new Exception("The snpDataByChromMap is null")
        };
    ...
    List<Long> getPatientNumListFromSubjectIdStr(String subjectIdStr) {
        if (subjectIdStr == null || subjectIdStr.length() == 0) {
            return null
        };
    ...
    String getConceptDisplayName(String conceptId, Map<String, String> conceptIdToDisplayNameMap) {
        if (conceptId == null || conceptId.length() == 0 || conceptIdToDisplayNameMap == null) {
            return null
        };
    ...
    void getGeneWithSnpMapForGenes(Map<String, SortedMap<Long, Map<Long, GeneWithSnp>>> geneSnpMapByChrom,
        ...
        if (geneSnpMapByChrom == null) {
            throw new Exception("geneSnpMapByChrom is not instantiated")
        };
        if (geneEntrezIdMap == null) {
            throw new Exception("geneEntrezIdMap is not instantiated")
        };
        ...
        for (Map.Entry geneEntry : geneEntrezIdMap) {
            ...
            if (gene.chrom == null || gene.snpMap == null || gene.snpMap.size() == 0) {
                continue
            };
    ...
    void getSnpDatasetBySubjectMap(Map<Long, SnpDataset[]> snpDatasetBySubjectMap, String subjectListStr) {
        if (snpDatasetBySubjectMap == null || subjectListStr == null || subjectListStr.length() == 0) {
            return
        };
    ...
    void getPatientGenderMap(String subjectListStr, Map<Long, String> patientGenderMap) {
        if (patientGenderMap == null) {
            throw new Exception("The object patientGenderMap is not instantiated")
        };
                    ...
                    if (snpDatasetPair != null) {
                        ...
                        if (snpDatasetPair[0] != null) {
                            ...
                            if (snpDatasetPair[1] != null) {
                                sampleInfoBuf.append("Yes")
                            } // Paired
                            else {
                                sampleInfoBuf.append("No")
                            }; // Not paired
                        }
                        if (snpDatasetPair[1] != null) {
                            // Has the control dataset
                            ...
                            if (snpDatasetPair[0] != null) {
                                sampleInfoBuf.append(datasetNameForSNPViewer_1)
                            } // Paired
                            else {
                                sampleInfoBuf.append("No")
                            }; // Not paired
            ...
            for (Map.Entry geneMapEntry : geneMapChrom) {
                ...
                for (Map.Entry geneEntry : geneMap) {
                    ...
                    for (Map.Entry snpEntry : snpMap) {
                        ...
                        if (snpNameSet != null && snpNameSet.contains(snp.name)) {
    ...
    def runPlink(GwasFiles gwasFiles) {
        if (gwasFiles == null) {
            throw new Exception("The object GwasFiles is not instantiated")
        };
    ...
    def reportGwas(String userName, GwasFiles gwasFiles, String querySummary1, String querySummary2) {
        ...
        if (assocFile == null || assocFile.isFile() == false) {
            throw new Exception("The PLINK output .assoc file does not exist")
        };
        ...
        TreeMap<Double, Set<String>> entrezScoreNegativeMap = new TreeMap<Double, String>();
    ...
    void getSnpGeneGwasScore(Map<String, String[]> snpNameDataMap, SortedMap<Double, Set<String>> entrezScoreNegativeMap,
        ...
        if (snpNameDataMap == null) {
            throw new Exception("The object snpNameDataMap is not instantiated")
        };
        if (snpNameDataMap.size() == 0) {
            return
        };
        if (entrezScoreNegativeMap == null) {
            throw new Exception("The object entrezScoreMap is not instantiated")
        };
        if (entrezSnpMap == null) {
            throw new Exception("The object entrezSnpMap is not instantiated")
        };
        if (snpEntrezMap == null) {
            throw new Exception("The object snpEntrezMap is not instantiated")
        };
        if (entrezNameMap == null) {
    ...
    Map<Long, Map<String, String>> getSNPDataByDatasetByChrom(String subjectIds, String chroms) throws Exception {
        if (subjectIds == null || subjectIds.trim().length() == 0) {
            return null
        };
    ...
    List<String> getSortedChromList(Set<String> chromSet) {
        if (chromSet == null || chromSet.size() == 0) {
            return null
        };
    ...
    String getSNPPlatform(String subjectIds1, String subjectIds2) throws Exception {
        ...
        if (platformList1 != null) {
            if (platformList1.size() == 1) {
                platform = platformList1.get(0)
            } else {
    ...
    void fillCensoringToSurvivalData(Concept conceptCensoring, Map<String, SurvivalData> dataMap, List<String> subjectStrList) throws Exception {
        ...
        sql.eachRow(sqlt, [
            ...
            if (censoringStr != null &&
                    (censoringStr.equalsIgnoreCase("Censoring") || (censoringStr.equalsIgnoreCase("Yes")))) {
    ...
    void fillEventToSurvivalData(Concept conceptEvent, Map<String, SurvivalData> dataMap, List<String> subjectStrList) throws Exception {
        ...
        sql.eachRow(sqlt, [conceptEvent.getBaseCode()], { row ->
            ...
            String censoringStr = row.tval_char;
            if (censoringStr != null &&
                    (censoringStr.equalsIgnoreCase("Event")) || censoringStr.equalsIgnoreCase("No")) {
                // This patient is censored
                SurvivalData survivalData = dataMap.get(subjectId);
                if (survivalData != null) {
    ...
    def String getSubjectIds1(String ids1, String ids2, String prefix1, String prefix2) {
        ...
        if (ids1 != null && ids1.length() > 0) {
            ...
            for (id in idArray) {
                s.append("'" + prefix1 + id + "' as " + prefix1 + id + ",")
            };
        }
        ...
        if (ids2 != null && ids2.length() > 0) {
            ...
            for (id in idArray) {
                s.append("'" + prefix2 + id + "' as " + prefix2 + id + ",")
            };
        }
    ...
    Map<String, String> getSecureTokensWithAccessForUser(AuthUser user) {
        def rows = AuthUserSecureAccess.executeQuery("""
            SELECT DISTINCT
                so.bioDataUniqueId,
                ausa.accessLevel.accessLevelName
    ...
    def fillHeatmapValidator(subids, conids, hv) {
        ...
        sql.eachRow(sqlt, { row ->
            ...
            if (row.SAMPLE_TYPE_CD != null) {
                hv.samples.add(row.SAMPLE_TYPE_CD)
            };
            if (row.SAMPLE_TYPE != null) {
                hv.sampleLabels.add(row.SAMPLE_TYPE)
            };
            if (row.TISSUE_TYPE_CD != null) {
                hv.tissues.add(row.TISSUE_TYPE_CD)
            };
            if (row.TISSUE_TYPE != null) {
                hv.tissueLabels.add(row.TISSUE_TYPE)
            };
        })
        if (hv.validate()) {
            return;
